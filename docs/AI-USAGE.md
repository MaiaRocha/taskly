...
- **Task Activity History (Fase 11)** — descrita em detalhe abaixo.
- **DemoSeeder** — descrita em detalhe abaixo, por ser a fase mais recente e com contexto completo disponível.
...
**DemoSeeder**, planejada com assistência de IA e revisada tecnicamente antes de qualquer implementação (inspeção do domínio real — models, factories, Actions, Policies, `RecordTaskActivity` — antes de propor a abordagem), executada em checkpoints (limpeza do `DatabaseSeeder`; config/credenciais + usuário demo; Projects/Tags; Tasks + Activity History; ajuste de conteúdo para um cenário de equipe de desenvolvimento; fechamento), cobriu: remoção da criação automática do "Test User" padrão do Laravel (`DatabaseSeeder` ficou neutro, produção nasce vazia com só `migrate --force`); um `DemoSeeder` explicitamente opt-in (`php artisan db:seed --class=DemoSeeder --force`, nunca chamado por `DatabaseSeeder`); credenciais lidas via `config/demo.php`/`.env.example` (nunca `env()` direto no Seeder), validadas com `Illuminate\Validation\Rules\Password::default()` — a mesma política já usada pelo Fortify — antes de qualquer escrita; idempotência por abortar (aviso + retorno, sem alterar nada) quando o usuário demo já existe, em vez de sincronizar/atualizar dados existentes; toda a criação (User, Projects, Tags, Tasks, pivots, Activities) dentro de uma única `DB::transaction`, com uma única captura de `$now` como referência para todas as datas relativas do dataset; um Activity History gerado por mutações reais contra Eloquent + `RecordTaskActivity` (nunca uma Activity inserida artificialmente), incluindo uma task-showcase com narrativa completa (criação → prazo → tags → status, 4 Activities) e outra deixada deliberadamente "fresca" para demonstração manual em vídeo; um dataset determinístico e curado (sem Faker) contextualizado como uma equipe real de desenvolvimento de software (projetos, tags e tarefas de produto/release/qualidade); testes automatizados cobrindo contagens exatas, distribuição de status, `overdue`/`completed_at`, coerência do Activity History, idempotência e rollback atômico; e validação humana com smoke test manual no navegador antes do fechamento.
